...
def pay(request):
    if request.method == 'POST':
        ...
        cburl = 'http://16.16.139.184/callback/'
        ref = str(uuid.uuid4())
        profile = Profile.objects.get(user__username=request.user.username)
...
def search(request):
    if request.method == 'POST':
        ...
        searched = Q(Q(title__icontains=items) | Q(description__icontains=items) |Q(category__title__icontains=items) |Q(category__title__icontains=items)
                     |Q(slug__icontains=items) | Q(price__icontains=items))
